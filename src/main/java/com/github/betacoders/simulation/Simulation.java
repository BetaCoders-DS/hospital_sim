...
public final class Simulation {
  ...
  private void trySpawn(double t) {
    ...
    Patient p = new Patient(new Position(genPos.x, genPos.y),
        rng.nextDouble() < cfg.preferentialP(), generateVitals());
    p.spawnTime(t);
    p.target(totem);
  ...
  private void checkIdleStations() {
    if (nurse.isIdle()) {
      Patient next = nurse.callNext(triageQueue);
      if (next != null) {
        inTriage = next;
    ...
    if (medic.isIdle()) {
      Patient next = medic.callNext(medicQueue);
      if (next != null) {
        inConsult = next;
  ...
  private void movePatients() {
    ...
    for (Patient p : patients) {
      switch (p.getState()) {
        case GOING_TO_TOTEM, WAITING_FOR_TRIAGE, WAITING_FOR_MEDIC, GOING_TO_TRIAGE, GOING_TO_MEDIC, GOING_TO_REMOVER ->
            moving.addLast(p);
        case AT_TOTEM, IN_TRIAGE, IN_CONSULTATION, REMOVED -> {}
      }
    }
    LinkedList<MoveIntention> intentions = Movement.computeIntentions(moving, distanceSource, occupancy);
  ...
  private SeatSpot nearestFreeSeat(Position from) {
    ...
    while (!queue.isEmpty()) {
      ...
      if (e instanceof StaticEntities.Seat s && s.isFree()) {
